...
CREATE TABLE IF NOT EXISTS `sduty`.`story` (
  ...
  `story_regtime` DATETIME NOT NULL,
...
CREATE TABLE IF NOT EXISTS `sduty`.`comment` (
  ...
  `comment_regtime` DATETIME NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS `sduty`.`study` (
  ...
  `study_is_camstudy` VARCHAR(45) NOT NULL DEFAULT 0,
  `study_regtime` DATETIME NOT NULL,
  `study_join_number` VARCHAR(45) NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS `sduty`.`alarm` (
  `alarm_seq` INT NOT NULL,
  `alarm_study_seq` INT NOT NULL,
  ...
  INDEX `alarm_study_seq_idx` (`alarm_study_seq` ASC) VISIBLE,
  CONSTRAINT `alarm_study_seq`
    FOREIGN KEY (`alarm_study_seq`)
    REFERENCES `sduty`.`study` (`study_seq`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS `sduty`.`daily_question` (
  `dailyq_seq` INT NOT NULL,
  `dailyq_admin_seq` INT NOT NULL,
  `dailyq_content` VARCHAR(200) NOT NULL,
  `dailyq_regtime` DATETIME NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS `sduty`.`qna` (
  ...
  `ques_regtime` DATETIME NOT NULL,
  `ans_content` VARCHAR(100) NOT NULL,
  `ans_writer` INT NOT NULL,
  `ans_regtime` DATETIME NOT NULL,
  PRIMARY KEY (`qna_seq`),
...
CREATE TABLE IF NOT EXISTS `sduty`.`task` (
  ...
  `task_duration_time` DATETIME NOT NULL,
